...
def save_snippet_audio(
    ...
    """Persist every snippet master longer than five seconds as mono PCM WAV."""
    ...
    try:
        ...
        _debug(f"Saved snippet master: {path} ({sample_rate} Hz)")
        return path
    except Exception as exc:
...
def transcribe(
    ...
    *,
    retain_snippet_audio: bool = False,
) -> str:
    ...
    if retain_snippet_audio:
        # Preserve the native USB Headset WASAPI capture before network access or
        # transcription can fail. This is the voice-cloning source of truth.
