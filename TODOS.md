...
**Why:** On 2026-03-17, Jordan's enrolled embedding scored 0.06-0.10 against his own voice over loopback (threshold 0.25) for every chunk. The embedding was enrolled from mic audio, but loopback audio has very different acoustic characteristics after WASAPI capture + resampling + normalization. There was no indication during the meeting that the embedding was useless — it silently fell through to "Speaker 1" every time.

**Pros:** Early detection of stale/incompatible embeddings. Could prompt mid-meeting notification like "Jordan's voice hasn't matched in 5 chunks — embedding may be stale." Saves the user from discovering the problem only at the end-of-meeting enrollment dialog.
