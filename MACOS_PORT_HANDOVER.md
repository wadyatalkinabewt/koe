...
## Project Location
`C:\Projects\koe\` - Python app with PyQt5 UI for speech-to-text (hotkey snippets) and meeting transcription (Scribe) with speaker diarization.
...
- Hardcoded debug log path `r"C:\Projects\koe\capture_debug.log"` should use `Path(__file__).parent.parent.parent / "capture_debug.log"`
...
Create at `C:\Projects\koe\requirements-mac.txt`:
```
