...
def test_successful_snippet_audio_is_saved_when_enabled(tmp_path, monkeypatch):
    import transcription
    ...
    def fake_config(*keys):
        values = {
            ("recording_options", "save_audio"): True,
            ("misc", "snippets_folder"): str(snippets_dir),
        }
        return values.get(tuple(keys))

    monkeypatch.setattr(transcription.ConfigManager, "get_config_value", fake_config)
    monkeypatch.setattr(
        transcription,
        "transcribe_elevenlabs",
        lambda _audio, sample_rate: "Saved audio",
    )
    monkeypatch.setattr(transcription, "save_rolling_transcription", lambda _text: None)
    monkeypatch.setattr(transcription, "save_transcription_debug", lambda *_args: None)

    audio = np.resize(np.arange(1000, dtype=np.int16), 16 * 16000)
    assert transcription.transcribe(audio, sample_rate=16000) == "Saved audio. "

    audio_dir = snippets_dir / "Audio Files"
    saved = list(audio_dir.glob("snippet_*.wav"))
    assert len(saved) == 1
...
def test_unsuccessful_or_disabled_snippet_audio_is_not_saved(tmp_path, monkeypatch):
    import transcription
    ...
    def fake_config(*keys):
        values = {
            ("recording_options", "save_audio"): False,
            ("misc", "snippets_folder"): str(snippets_dir),
        }
        return values.get(tuple(keys))
...
def test_snippet_audio_at_or_below_fifteen_seconds_is_not_saved(
    tmp_path,
    monkeypatch,
):
    import transcription
    ...
    monkeypatch.setattr(
        transcription,
        "transcribe_elevenlabs",
        lambda _audio, sample_rate: "Short but successful",
    )
    monkeypatch.setattr(transcription, "save_rolling_transcription", lambda _text: None)
    monkeypatch.setattr(transcription, "save_transcription_debug", lambda *_args: None)

    assert transcription.transcribe(
        np.ones(15 * 16000, dtype=np.int16),
        sample_rate=16000,
